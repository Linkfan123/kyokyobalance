...
SWEP.AirMult					 = 1.5
SWEP.ShootMult					 = 1.10
SWEP.BaseCone					 = 0.02 -- ALWAYS SET TO WHAT YOU SET SWEP.Primary.Cone AS
SWEP.MaxShootPenalty			 = SWEP.BaseCone * 3 -- it's good to make this 3x what the OG cone is
...
function SWEP:ShootBullet( dmg, recoil, numbul, cone )
   ...
   self:GetOwner():FireBullets( bullet )
	...
	-- ramp down accuracy as you spray and pray
	if self.ShootMult != 1 then
		self.Primary.Cone = self.Primary.Cone * self.ShootMult -- does the actual thing
		if self.Primary.Cone > self.MaxShootPenalty then self.Primary.Cone = self.MaxShootPenalty end -- makes sure you don't get too wild
		timer.Create("reset", self.Primary.Delay, 1, function() 
			self.Primary.Cone = self.BaseCone -- resets the cone back to its OG
		end )
	end
...
function SWEP:GetPrimaryCone()
   local cone = self.Primary.Cone or 0.2
	...
	-- decrease accuracy when in the air
   if !ply:IsOnGround() then cone = cone * self.AirMult end
	-- increase accuracy with ironsights
	if self:GetIronsights() then cone = cone * self.SightMult end
	-- increase accuracy while crouching
   if ply:Crouching() and ply:IsOnGround() then cone = cone * self.CrouchMult end
	return cone
end
